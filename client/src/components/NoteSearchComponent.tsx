...
const NoteSearchComponent = ({ unitId, facultyColor }) => {
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      try {
        setIsSearching(true);
        const response = await apiGet(`/notes/search?q=${encodeURIComponent(debouncedSearchTerm)}&unit_id=${unitId}`);
  ...
  const handleNoteClick = (noteId, matchIndex = 0) => {
    navigate(`/notes/${noteId}?query=${encodeURIComponent(searchTerm)}&matchIndex=${matchIndex}`);
    setShowSuggestions(false);
  };
                          ...
                          <div>
                            ...
                            {topMatch && (
                              <div className="mt-1 text-sm text-gray-600">
                                <div>
                                  <Badge size="sm" variant="outline" className="mr-2 text-xs">
                                    Page {topMatch.page}
                                  </Badge>
