...
// import { Course } from '../types'; // Type not available
type Course = any; // Temporary fix for build
        ...
        <div className="h-full flex flex-col">
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            <Link
              href="/"
              className={pathname === '/' 
                ? "flex items-center px-3 py-2 rounded-md bg-indigo-50 text-indigo-600"
                : "flex items-center px-3 py-2 rounded-md text-gray-700 hover:bg-gray-100"
              }
            >
            ...
            <div>
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-md ${
                  pathname.includes('/courses') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-gray-100'
                }`}
              >
                <div className="flex items-center">
              ...
              {coursesExpanded && (
                <div className="ml-6 mt-1 space-y-1">
                  ...
                  {courses.map(course => (
                    <Link
                      key={course._id}
                      href={`/courses/${course._id}/units`}
                      className={pathname === `/courses/${course._id}/units`
                        ? "flex items-center px-3 py-2 rounded-md bg-indigo-50 text-indigo-600"
                        : "flex items-center px-3 py-2 rounded-md text-gray-700 hover:bg-gray-100"
                      }
                    >
